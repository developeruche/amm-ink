...
/// this would be useful fir mathematical operations
...
mod amm {
    ...
    /// Here comes the error enum, this would be stating all the error messages the AMM contract would be throwing
    ...
    /// Looking at the storage structure, it is very similar to the uniswap storage struct, the amm model would a constant product AMM (x*y = k)
    #[derive(Default)]
    #[ink(storage)]
    pub struct Amm {
    ...
    #[ink(impl)] // this attribute iis here to tell ink that the is an implementation and should treat it as such 
    impl Amm {
        // Ensures that the _qty is non-zero and the user has enough balance
        fn validAmountCheckShare(
            &self,
            _qty: Balance,
        ) -> Result<(), Error> {
            let caller = self.env().caller();
            let my_balance = self.shares.get(&caller).unwrap_or(0);
        ...
        // Ensures that the _qty is non-zero and the user has enough balance
        fn validAmountCheckToken1(
            &self,
            _qty: Balance,
        ) -> Result<(), Error> {
            let caller = self.env().caller();
            let my_balance = self.token1Balance.get(&caller).unwrap_or(0);
        ...
        // Ensures that the _qty is non-zero and the user has enough balance
        fn validAmountCheckToken2(
            &self,
            _qty: Balance,
        ) -> Result<(), Error> {
            let caller = self.env().caller();
            let my_balance = self.token2Balance.get(&caller).unwrap_or(0);
    ...
    impl Amm {
        ...
        pub fn provide(
            ...
            self.validAmountCheckToken1(_amountToken1)?;
            self.validAmountCheckToken2(_amountToken2)?;
            ...
            if self.shares.get(&caller).unwrap_or(0) == 0 {
                ...
                self.shares.insert(&caller, &(self.shares.get(&caller).unwrap_or(0) + share));
            }
        ...
        pub fn withdraw(&mut self, _share: Balance) -> Result<(Balance, Balance), Error> {
            let caller = self.env().caller();
            self.validAmountCheckShare(_share)?;

            let (amountToken1, amountToken2) = self.getWithdrawEstimate(_share)?;
            self.shares.insert(&caller, &(self.shares.get(&caller).unwrap_or(0) - _share));
            self.totalShares -= _share;

            self.totalToken1 -= amountToken1;
            self.totalToken2 -= amountToken2;
            self.token1Balance.insert(&caller, &(self.token1Balance.get(&caller).unwrap_or(0) + amountToken1));
            self.token2Balance.insert(&caller, &(self.token2Balance.get(&caller).unwrap_or(0) + amountToken2));
        ...
        pub fn swapToken1GivenToken1(
            ...
            self.token1Balance.insert(&caller, &(self.token1Balance.get(&caller).unwrap_or(0) - _amountToken1));
        ...
        pub fn swapToken1GivenToken2(
            ...
            self.validAmountCheckToken1(amountToken1)?;
            ...
            self.token2Balance.insert(&caller, &(self.token2Balance.get(&caller).unwrap_or(0) + _amountToken2));
